...
def main():
    ...
    with open("results/cnn_ratio_results.pk", "rb") as f:
    ...
    params_log, _ = curve_fit(log_func, sizes, iqms)
    a_log, b_log = params_log
    fit_log = log_func(sizes, a_log, b_log)
    ...
    plt.plot(
        ...
        color="black",
        linewidth=1.5,
        label=rf"${a_log:.4f} \log(n) + {b_log:.4f}$",
    )
