...
from cstp import graph_utils, christofides_p
from .types import *
...
def canadian_traveller_cyclic_routing(G: nx.Graph, blocked: Path, tour: Edge_Path = []) -> tuple[nx.Graph, Weight]:
    ...
    if tour == []:
        tour = christofides_p.travelling_salesman_christofides(G)[0]
...
def get_node_tour(i: int, tour: list[tuple[int, Node]]) -> tuple[int, Node]:
    ...
    for j, v in tour:
        if i == j:
            return (i, v)
    return None
>>>>>>> 76cb8331080443a512b3925c0d196910adc6fdf7:cctp/cr.py
